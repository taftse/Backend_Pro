<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Page
	{
		 ...
		 */
		function _write_cache ($path, $area, $type)
		{
			// Check dir is valid and writeable
			if ( ! is_dir ($path) OR ! is_writable ($path))
			{
				log_message('error','Cache path (' . $path . ') is not a directory or is not writable');
				return FALSE;
			}

			if(count($this->default_assets[$area][$type]) == 0)
			{
			...
			$path = $path . $this->CI->config->item('asset_cache_file_pfx') . ($this->CI->config->item('asset_cache_length')*3600 + time()) . "." . $type;

			// Foreach file belonging to $area & $type add it to the cache file
			$output = "";
			foreach($this->default_assets[$area][$type] as $file)
			{
				$file_path =  $this->CI->config->item($area . "_assets") . $type . "/" . $file;
				ob_start();
				//include base_url() . $file_path;
				include $file_path;
			...
			$output = $this->_cache_compress($output);

			// Write the cache file and link it
			$this->{'_include_' . $type}($path);
